...
clk,rst_n,hz1out,hz4out,hz64out,hz500out,hz1000out
    ...
    parameter hz1=4,hz4=25000000,hz64=1562500,hz500=200000,hz1000=100000;
   //1 
...
module controlcircuit(
    ...
    output reg toSecond,toMinute,toHour,setZero;
...
output reg [3:0] minH
    ...
    else if(minH==5&&minL==9) //计数�?59min
        begin
        {minH,minL}<=8'd0;
        clkout <= 1;
        end
...
output reg [3:0] hourH
    ...
    else if(hourH==2&&hourL==3) //计数�?23hour
        {hourH,hourL}<=8'd0;
...
Clk,SecondCarry,Control,countPlus
   ...
   always@(Control,Clk,SecondCarry)
   case(Control)
   1'b1: countPlus=Clk;
   1'b0: countPlus=SecondCarry;
   endcase
